package com.ms.seckill.interceptor;
...
public class SeckillInterceptor implements HandlerInterceptor {
    ...
    public static final ThreadLocal<String> THREAD_LOCAL_SECKILL = new ThreadLocal<>();
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (StringUtils.hasText(claim)) {
            THREAD_LOCAL_SECKILL.set(claim);
            return true;
        }
        log.warn("用户尚未认证，不可进入秒杀服务");
        return false;
    }
}
